...
- **Custom Vocabulary File (`--custom-vocabulary-file`)**: Register company-internal / frequently-misrecognized terms in a plain text file (one per line) and the wrapper biases the transcript toward those terms as a post-recognition step. Defaults to `auto` which automatically picks up `.vocab.txt` (or `<stem>.vocab.txt`) when present. Up to 1000 words are accepted by the model; Google recommends ≤100 lines for best results. See [Custom Vocabulary](docs/custom-vocabulary.md) for details.
