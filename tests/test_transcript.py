...
def run():
    with tempfile.TemporaryDirectory() as td:
        ...
        print("outputs regenerated:", sorted(p.name for p in td.glob("input.diarized.srt")) + sorted(p.name for p in td.glob("input.srt")) + sorted(p.name for p in td.glob("input.txt")))
        print("re-render status:", r2.results[0].status)
        assert API_CALLS["n"] == 1, f"re-render should not call API, calls={API_CALLS['n']}"
        assert (td / "input.diarized.srt").exists() and (td / "input.srt").exists() and (td / "input.txt").exists()
